...
function createMap(addressInfo=null) {
    ymaps.ready(function () {
        ...
        if (!map) {
            ...
            var geolocationControl = new ymaps.control.GeolocationControl({
                options: {
                    layout: ymaps.templateLayoutFactory.createClass(
                        '<div id="geolocation" class="v-map-custom-controls--geolocation"><svg viewBox="0 0 24 24" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M19.925 1.78443C21.5328 1.18151 23.1029 2.75156 22.5 4.35933L16.0722 21.5C15.3574 23.4061 12.5838 23.1501 12.23 21.1453L10.8664 13.418L3.1391 12.0544C1.13427 11.7006 0.878261 8.92697 2.78443 8.21216L19.925 1.78443ZM20.6273 3.65708L3.48668 10.0848L11.2139 11.4485C12.0417 11.5945 12.6898 12.2426 12.8359 13.0704L14.1996 20.7977L20.6273 3.65708Z" fill="#0F0F0F"/></svg></div>'
                    ),
                    position: {
                        bottom: '190px',
                        right: '20px'
            ...
            if (mapContainer.getAttribute('data-full-screen')){
                ...
                closeButton = new ymaps.control.Button({     
                    options: {
                        // noPlacemark: true,
                        layout: ymaps.templateLayoutFactory.createClass(
                            '<button type="button" data-test-id="delivery-map-close-btn" class="v-button v-button--small justify-center shrink"><span class="v-button__wrapper" style="opacity: 1;"><span class="d-flex"><svg heigh="24px" width="24px" stroke="#000" viewBox="0 2 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M15 7L10 12L15 17" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></span></span></button>'
                        ),
                        float: 'left',
                    }
                });
                ...
                cleanButton = new ymaps.control.Button({     
                    options: {
                        // noPlacemark: true,
                        layout: ymaps.templateLayoutFactory.createClass(
                            '<button type="button" data-id="delivery-map-clean-btn" class="v-button v-button--small justify-center shrink"><span class="v-button__wrapper" style="opacity: 1;"><span class="d-flex"><svg width="24" height="24" stroke="#000000" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4 7H20" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 10L7.70141 19.3578C7.87432 20.3088 8.70258 21 9.66915 21H14.3308C15.2974 21 16.1257 20.3087 16.2986 19.3578L18 10" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></span></span></button>'
                        ),
                        float: 'right',
                    },            
                    state: {
            ...
            store = new ymaps.Placemark(
                [56.050918, 92.904378], {
                    ...
                    iconImageHref: '/static/svg/map/mikado-map.svg',
                    // Размеры метки.
                    iconImageSize: [70, 70],
        ...
        if ($(address_line1).attr('captured')){
            if (cleanButton){
                cleanButton.state.set({enabled:true});
            }
            $(saveButton).attr('disabled', true);
        } else {
            $(deliveryControls).addClass('d-none');
            if (cleanButton){
                cleanButton.state.set({enabled:false});
            }
