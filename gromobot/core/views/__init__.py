from django.shortcuts import render


# pylint: disable=unused-argument
def handler403(request, exception):
    return render(
        request, "webshop/403.html", {"page_title": "Доступ запрещен"}, status=403
    )


# pylint: disable=unused-argument
def handler404(request, exception):
    return render(
        request, "webshop/404.html", {"page_title": "Страница не найдена"}, status=404
    )


def handler500(request):
    return render(
        request, "webshop/500.html", {"page_title": "Тахнические неполадки"}, status=500
    )


def sort_queryset(queryset, request, allowed_sorts, default=None):
    """Sorts the queryset by one of allowed_sorts based on parameters
    'sort' and 'dir' from request"""
    sort = request.GET.get("sort", None)
    if sort in allowed_sorts:
        direction = request.GET.get("dir", "asc")
        sort = ("-" if direction == "desc" else "") + sort
        queryset = queryset.order_by(sort)
    elif default:
        queryset = queryset.order_by(default)
    return queryset
