...
class CheckoutForm(forms.ModelForm):
    ...
    order_note = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "class": "input d-flex align-center input__padding text-area",
                "rows": 2,
            }
        ),
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.fields["method_code"].choices = ((m.code, m.name) for m in methods)
        if self.fields["method_code"].choices:
            self.fields["method_code"].initial = self.fields["method_code"].choices[0][
                0
            ]
