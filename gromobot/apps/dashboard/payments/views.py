...
from apps.webshop.payment.exceptions import DebitedAmountIsNotEqualsRefunded
...
PaymentManager = get_class("webshop.payment.methods", "PaymentManager")
...
logger = logging.getLogger("apps.dashboard.payments")


class TransactionsListView(SingleTableView):
    template_name = "dashboard/payments/transaction_list.html"
    context_table_name = "transactions"
    paginate_by = settings.DASHBOARD_PAYMENTS_PER_PAGE
...
class RefundDetailView(DetailView):
    template_name = "dashboard/payments/refund_detail.html"
    model = Refund
        ...
        try:
            for trx in self.object.transactions.all():
                ...
                if pay_id:
                    ...
                    payment_method = PaymentManager(
                        source_reference, request.user
                    ).get_method()
            ...
            logger.error(
                "Ошибка при обновлении транзакции: №{0}, Пользователь: {1}, Ошибка: {2}".format(
...
class DeleteSourceView(DeleteView):
    ...
    template_name = "dashboard/orders/confirm_delete_source.html"
...
class AddSourceView(FormView):
    ...
    template_name = "dashboard/orders/add_source.html"
    ...
    def form_valid(self, form):
        try:
            ...
            logger.error(
                "Ошибка при добавлении способа оплаты для заказа: №{0}, Пользователь: {1}, Ошибка: {2}".format(
...
class AddTransactionView(CreateView):
    template_name = "dashboard/orders/add_transaction.html"
    model = Transaction
    order_model = Order
    ...
    def form_valid(self, form):
        try:
            ...
            logger.error(
                "Ошибка создание транзакции для источника оплаты: {0}, Пользователь: {1}, Ошибка: {2}".format(
...
class RefundTransactionView(DetailView):
    ...
    template_name = "dashboard/orders/confirm_refund.html"
