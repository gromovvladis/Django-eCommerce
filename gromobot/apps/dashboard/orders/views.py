...
from apps.webshop.order import exceptions as order_exceptions
from apps.webshop.payment.exceptions import PaymentError
...
from django.db.models import (Avg, Case, Count, DurationField, Exists,
                              ExpressionWrapper, F, Max, OuterRef, Q, Sum,
                              Value, When, fields)
...
EventHandlerMixin = get_class("webshop.order.mixins", "EventHandlerMixin")
...
logger = logging.getLogger("apps.dashboard.orders")
...
def queryset_orders(request, product=None, category=None):
    ...
    queryset = Order._default_manager.select_related(
        ...
        "lines", "status_changes", "sources", "payment_events", "shipping_events"
    )
...
class OrderStatsView(FormView):
    ...
    def get_report(self, orders, start, end, range_type):
        ...
        if range_type == "days":
            ...
            raise ValueError(
                "Invalid range_type. Must be 'days', 'weeks', 'months', or 'years'."
            )
        ...
        for _ in range(range_time):
            ...
            if range_type == "days":
                ...
                order_labels.append(
                    start_time.strftime("%d.%m") + "-" + end_time.strftime("%d.%m")
                )
            elif range_type == "months":
    ...
    def get_data(self, filters, excludes):
        orders = (
            queryset_orders(request=self.request).filter(**filters).exclude(**excludes)
        )
        ...
        if filters.get("date_placed__range") is not None:
            ...
            alerts = StockAlert.objects.filter(
                ...
                stockrecord__store__in=self.request.staff_stores,
            )
            baskets = Basket.objects.filter(
                status=Basket.OPEN,
                date_created__range=[start_date, end_date],
                store__in=self.request.staff_stores,
            )
            ...
            baskets = Basket.objects.filter(
                status=Basket.OPEN, store__in=self.request.staff_stores
            )
        ...
        data = {
            ...
            "customers": users.filter(
                orders__store__in=self.request.staff_stores
            ).distinct(),
            "lines": lines,
            "products": products,
        }
    ...
    def get_stats(self, filters, excludes):
        ...
        for period, start in start_dates.items():
            ...
            top_products = (
                lines_period.values("product", "name")
                .annotate(total_quantity=Sum("quantity"), total_sum=Sum("line_price"))
                .order_by("-total_quantity")[:5]
            )

            revenue = aggregate_orders(orders_period, "total", Sum)
            revenue_offline = aggregate_orders(
                orders_period.filter(site="Эвотор"), "total", Sum
            )
            revenue_online = revenue - revenue_offline

            discount = (
                aggregate_orders(
                    orders_period, "lines__line_price_before_discounts", Sum
                )
                - revenue
            )
            discount_offline = (
                aggregate_orders(
                    orders_period.filter(site="Эвотор"),
                    "lines__line_price_before_discounts",
                    Sum,
                )
                - revenue_offline
            )
            ...
            stats.update(
                {
                    ...
                    f"orders_offline_{period}": orders_period.filter(
                        site="Эвотор"
                    ).count(),
                    f"orders_online_{period}": orders_period.exclude(
                        site="Эвотор"
                    ).count(),
                    ...
                    f"average_costs_{period}": aggregate_orders(
                        orders_period, "total", Avg
                    ),
                    f"average_offline_costs_{period}": aggregate_orders(
                        orders_period.filter(site="Эвотор"), "total", Avg
                    ),
                    f"average_online_costs_{period}": aggregate_orders(
                        orders_period.exclude(site="Эвотор"), "total", Avg
                    ),
                    f"alerts_{period}": alerts.filter(
                        date_created__range=(start_date, end_date)
                    ).count(),
                    f"baskets_{period}": baskets.filter(
                        date_created__range=(start_date, end_date)
                    ).count(),
                    f"users_{period}": users.filter(
                        date_joined__range=(start_date, end_date)
                    ).count(),
                    f"customers_{period}": customers.filter(
                        date_joined__range=(start_date, end_date)
                    ).count(),
                    f"customers_2orders_{period}": count_customers_with_orders(
                        customers, 2
                    ),
                    f"customers_5orders_{period}": count_customers_with_orders(
                        customers, 5
                    ),
                    f"items_{period}": lines_period.aggregate(items=Sum("quantity"))[
                        "items"
                    ]
                    or 0,
                    f"items_offline_{period}": lines_period.filter(
                        order__site="Эвотор"
                    ).aggregate(items=Sum("quantity"))["items"]
                    or 0,
                    f"items_online_{period}": lines_period.exclude(
                        order__site="Эвотор"
                    ).aggregate(items=Sum("quantity"))["items"]
                    or 0,
                    f"lines_{period}": lines_period.count(),
                    f"lines_offline_{period}": lines_period.filter(
                        order__site="Эвотор"
                    ).count(),
                    f"lines_online_{period}": lines_period.exclude(
                        order__site="Эвотор"
                    ).count(),
                    f"orders_discount_{period}": orders_period.filter(
                        ~Q(lines__line_price_before_discounts=F("lines__line_price"))
                    )
                    .distinct()
                    .count(),
                    f"orders_offline_discount_{period}": orders_period.filter(
                        ~Q(lines__line_price_before_discounts=F("lines__line_price")),
                        site="Эвотор",
                    )
                    .distinct()
                    .count(),
                    f"orders_online_discount_{period}": orders_period.filter(
                        ~Q(lines__line_price_before_discounts=F("lines__line_price"))
                    )
                    .exclude(site="Эвотор")
                    .distinct()
                    .count(),
                    f"lines_discount_{period}": lines_period.filter(
                        ~Q(line_price_before_discounts=F("line_price"))
                    ).count(),
                    f"lines_offline_discount_{period}": lines_period.filter(
                        ~Q(line_price_before_discounts=F("line_price")),
                        order__site="Эвотор",
                    ).count(),
                    f"lines_online_discount_{period}": lines_period.filter(
                        ~Q(line_price_before_discounts=F("line_price"))
                    )
                    .exclude(order__site="Эвотор")
                    .count(),
                    f"products_{period}": products.filter(
                        date_created__range=(start_date, end_date)
                    ).count(),
                    f"top_products_{period}": top_products,
                    f"order_status_breakdown_{period}": orders_period.order_by("status")
                    .values("status")
                    .annotate(freq=Count("id")),
                }
            )

            stats["top_products_names"].append(
                [product["name"] for product in top_products]
            )
            stats["top_products_quantities"].append(
                [product["total_quantity"] for product in top_products]
            )
            stats["top_products_sums"].append(
                [int(product["total_sum"]) for product in top_products]
            )

            report, stats[f"report_exist_{period}"] = self.get_report(
                orders, start, end_date, period
            )
...
class OrderListView(EventHandlerMixin, BulkEditMixin, SingleTableView):
    ...
    def get_table_pagination(self, table):
        return dict(per_page=settings.ORDERS_PER_PAGE)
    ...
    def get_queryset(self):
        ...
        if data["store"]:
            queryset = queryset.filter(store__code=data["store"])
        ...
        if data["username"]:
            queryset = queryset.filter(
                user__username__istartswith=data["username"]
            ).distinct()
    ...
    def get_search_filter_descriptions(self):
        ...
        if data.get("article"):
            descriptions.append(
                (
                    ...
                    # in an online store.
                    ('Включает товар с артикулом "{article}"').format(
        ...
        if data.get("date_from") and data.get("date_to"):
            ...
            descriptions.append(
                (
                    ...
                    ("Размещено после {start_date}").format(
        ...
        if data.get("is_online") and not data.get("is_offine"):
            descriptions.append((("Онлайн заказы"), (("is_online", True),)))

        if not data.get("is_online") and data.get("is_offine"):
            descriptions.append((("Эвотор заказы"), (("is_offline", True),)))
        ...
        if data.get("status"):
            descriptions.append(
                (
                    ...
                    ("Статус заказа {order_status}").format(
    ...
    def get_row_values(self, order):
        formatted_items = [
            f"{item.name}({item.quantity})" for item in order.lines.all()
        ]
        row = {
            "number": order.number,
            "total": order.total,
            ...
            "date_finish": (
                format_datetime(order.date_finish, "DATETIME_FORMAT")
            ...
            "shipping_address_name": (
                order.shipping_address.line1 if order.shipping_address else ""
            ),
        }
        return row
    ...
    def download_selected_orders(self, request, orders):
        ...
        response["Content-Disposition"] = (
            f"attachment; filename={self.get_download_filename(request)}"
        )
        output.close()
...
class OrderActiveListView(OrderListView):
    template_name = "dashboard/orders/active_order_list.html"
    ...
    def get_queryset(self):
        ...
        queryset = (
            sort_queryset(self.base_queryset, self.request, ["number", "total"])
...
class OrderActiveListLookupView(APIView):
    ...
    def get(self, request):
        ...
        if (
            num_orders == int(request.GET.get("order_num", 0))
            and request.GET.get("force", "false") == "false"
        ):
        ...
        html = render(request, self.template_name, {"table": table}).content.decode(
            "utf-8"
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            order_html = render_to_string(
                "dashboard/orders/partials/order-modal.html",
                {"order": order, "order_paid": order_paid, "lines": lines},
...
class OrderNextStatusView(EventHandlerMixin, APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {"next_status": order.next_status, "final": False}, status=200
            )
        except Exception:
...
class OrderDetailView(EventHandlerMixin, DetailView):
    ...
    def handle_line_action(self, request, order, action):
        ...
        if len(line_ids) == 0:
            return self.reload_page(
                error="Вы должны выбрать несколько строк для действий."
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["order_paid"] = sources.aggregate(
            paid=Sum(F("amount_debited") - F("amount_refunded"), default=0)
        )["paid"]
    ...
    def _create_payment_event(
        ...
        try:
            ...
            messages.error(
                request,
                "Невозможно создать платежное событие из-за ошибки платежа: %s" % e,
            )
...
class OrderDeleteView(DeleteView):
    ...
    def deletion_denied(self, order):
        ...
        elif order.status != settings.FAIL_ORDER_STATUS:
            messages.error(
                self.request,
                f"Переведите заказ в статус {settings.FAIL_ORDER_STATUS} перед удалением.",
            )
            return HttpResponseRedirect(
                reverse("dashboard:order-detail", kwargs={"number": order.number})
            )
