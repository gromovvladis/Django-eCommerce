...
var shippingTimeBtn = document.querySelectorAll('[data-id="shipping-time"]');
var shippingTimeLater = document.getElementById('shipping_time_later');

var shippingMethodButtons = document.querySelectorAll('[data-id="shipping-method-button"]');
var methodSwaper = document.querySelector('[data-id="shipping-method-block"]');
var timeSwaper = document.querySelector('[data-id="shipping-time-block"]');
...
shippingTimeBtn.forEach(function (time_btn) {
    time_btn.addEventListener('click', function () {
        ...
        shippingTimeMethod = time_btn.getAttribute("data-type");
        if (shippingTimeMethod === "now") {
            if (addressFields) {
                GetTime({ address: line1.value, coords: [lon.value, lat.value], shippingMethod: shippingMethod }).then(function (result) {
            ...
            shippingTimeLater.classList.add('hidden');
            shippingTimeLater.classList.remove('active');
        } else {
            shippingTimeLater.classList.remove('hidden');
            shippingTimeLater.classList.add('active');
...
function updateTimes() {
    if (shippingTimeMethod === "now" && shippingTimeLater.classList.contains("hidden")) {
        if (addressFields) {
            GetTime({ address: line1.value, coords: [lon.value, lat.value], shippingMethod: shippingMethod }).then(function (result) {
                orderTime.value = result.timeUTC;
                timeCaptured(result);
