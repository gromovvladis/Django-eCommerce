...
from apps.webshop.checkout.signals import post_checkout
from core.loading import get_class, get_model

OrderCreator = get_class("webshop.order.utils", "OrderCreator")
OrderDispatcher = get_class("webshop.order.utils", "OrderDispatcher")
CheckoutSessionMixin = get_class("webshop.checkout.session", "CheckoutSessionMixin")
OrderNumberGenerator = get_class("webshop.order.utils", "OrderNumberGenerator")
...
class OrderPlacementMixin(CheckoutSessionMixin):
    ...
    def handle_successful_order(self, order):
        ...
        if settings.SEND_ORDER_PLACED_EMAIL:
            try:
                self.send_order_placed_email(order)
            except Exception as e:
                logger.error(
                    "Невозможно отправить email у заказа номер №%s. Ошибка: %s",
                    order.number,
                    e,
                )
