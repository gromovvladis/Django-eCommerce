...
CheckoutForm = get_class("webshop.checkout.forms", "CheckoutForm")
UnableToPlaceOrder = get_class("webshop.order.exceptions", "UnableToPlaceOrder")
OrderPlacementMixin = get_class("webshop.checkout.mixins", "OrderPlacementMixin")
...
CheckoutVoucherForm = get_class("webshop.checkout.forms", "CheckoutVoucherForm")
...
logger = logging.getLogger("apps.webshop.checkout")


class IndexView(ThemeMixin, CheckoutSessionMixin, generic.FormView):
    """
...
class CheckoutView(PageTitleMixin, ThemeMixin, CheckoutSessionMixin, generic.FormView):
...
class PaymentDetailsView(PageTitleMixin, ThemeMixin, OrderPlacementMixin, generic.TemplateView):
    ...
    def _get_order_status(self, payment_method):
        if payment_method in settings.ONLINE_PAYMENTS:
            return settings.INITIAL_ONLINE_PAYMENT_ORDER_STATUS
        else:
