...
from core.loading import cached_import_string, get_class, get_classes, get_model
from core.models.img.image_processor import ImageProcessor
from core.models.img.paths import get_image_offers_upload_path

ExpandDownwardsCategoryQueryset = get_class(
    "webshop.catalogue.expressions", "ExpandDownwardsCategoryQueryset"
)
ActiveOfferManager, RangeManager, BrowsableRangeManager = get_classes(
    "webshop.offer.managers",
    ["ActiveOfferManager", "RangeManager", "BrowsableRangeManager"],
)
load_proxy, unit_price = get_classes(
    "webshop.offer.utils", ["load_proxy", "unit_price"]
)
ZERO_DISCOUNT = get_class("webshop.offer.results", "ZERO_DISCOUNT")
...
class ConditionalOffer(models.Model):
    ...
    max_discount = models.DecimalField(
        ...
        blank=True,
        help_text=(
            "Когда сумма всех скидок предложения достигнет этого значения,"
            "то предложение становится недоступным"
        ),
    )
...
class Benefit(BaseOfferMixin, models.Model):
    ...
    def proxy_map(self):
        return {
            self.PERCENTAGE: get_class(
                "webshop.offer.benefits", "PercentageDiscountBenefit"
            ),
            self.FIXED: get_class("webshop.offer.benefits", "AbsoluteDiscountBenefit"),
            self.FIXED_UNIT: get_class(
                "webshop.offer.benefits", "FixedUnitDiscountBenefit"
            ),
            self.MULTIBUY: get_class(
                "webshop.offer.benefits", "MultibuyDiscountBenefit"
            ),
            self.FIXED_PRICE: get_class("webshop.offer.benefits", "FixedPriceBenefit"),
    ...
    def clean_shipping_absolute(self):
        ...
        if self.range:
            errors.append(
                "Не следует выбирать диапазон, поскольку это преимущество не распространяется на товары."
            )
        if self.max_affected_items:
            errors.append(
...
class Condition(BaseOfferMixin, models.Model):
    """
    ...
    def proxy_map(self):
        return {
            self.COUNT: get_class("webshop.offer.conditions", "CountCondition"),
            self.VALUE: get_class("webshop.offer.conditions", "ValueCondition"),
            self.COVERAGE: get_class("webshop.offer.conditions", "CoverageCondition"),
        }
