...
from core.forms.mixins import PhoneNumberMixin
from core.forms.widgets import DatePickerInput, DateRangeInput
...
class OrderStatsForm(forms.Form):
    ...
    def _determine_filter_metadata(self):
        ...
        if username:
            ...
            self._search_filters.append(
                (
                    ('Номер клиента совпадает с "{name}"').format(name=username),
                    (("username", username),),
                )
            )
        ...
        if article:
            ...
            self._search_filters.append(
                (
                    ('Включает артикул "{prod_article}"').format(prod_article=article),
                    (("article", article),),
                )
            )
        ...
        if evotor_code:
            ...
            self._search_filters.append(
                (
                    ('Включает товар с кодом Эвотор. "{sku}"').format(sku=evotor_code),
        ...
        if voucher:
            ...
            self._search_filters.append(
                (
                    ('Использованный промокод "{code}"').format(code=voucher),
                    (("voucher", voucher),),
                )
            )
        ...
        if payment_method:
            ...
            self._search_filters.append(
                (
                    ("Оплачено с помощью {method}").format(method=payment_method.name),
        ...
        if is_online and not is_offine:
            ...
            self._search_filters.append((("Онлайн заказы"), (("is_online", True),)))

        if not is_online and is_offine:
            self._filters["site__in"] = settings.OFFLINE_ORDERS
            self._search_filters.append((("Эвотор заказы"), (("is_offline", True),)))
    ...
    def format_phone_number(self, phone_number):
        ...
        if len(phone_number) == 11 and phone_number.startswith("7"):
            phone_number = "+7 ({}) {}-{}".format(
                phone_number[1:4], phone_number[4:7], phone_number[7:]
            )
        return phone_number
...
class OrderSearchForm(forms.Form):
    ...
    date_from = forms.DateField(required=False, label="Дата с", widget=DatePickerInput)
    date_to = forms.DateField(required=False, label="Дата до", widget=DatePickerInput)
...
class NewTransactionForm(forms.ModelForm):
    ...
    code = forms.CharField(
        ...
        required=False,
        help_text='Код для интернет транзакций. Формат "0e000b000-000f-0000-a000-00000000ac00"',
    )
    paid = forms.BooleanField(
        initial=True, label="Оплачено", required=True, help_text="Заказ оплачен?"
    )
    refundable = forms.BooleanField(
        initial=True,
        label="Возврат возможен?",
        required=True,
        help_text="Оплату возможно вернуть?",
    )
