...
from apps.telegram.bot.const_texts import customers_text, report_text, staffs_text
from apps.telegram.bot.keyboards.inline.reports import report_keyboard
from apps.telegram.bot.models.report import (
    get_customers_message,
    get_report_message,
    get_staffs_message,
)
from apps.telegram.bot.models.user import check_staff_status
from apps.telegram.bot.states.states import StaffReport
...
async def report(message: Message, state: FSMContext):
    if await check_staff_status(message, state):
        ...
        await message.answer(
            "За какой период сформировать отчет?", reply_markup=report_keyboard
        )
...
async def report_week(callback: CallbackQuery, state: FSMContext):
    start_period = timezone.now().replace(
