...
from oscar.apps.evotor.client import EvatorCloud
...
logger = logging.getLogger("oscar.evotor")
...
def process_bulk_task(self, bulk_evotor_id):
    ...
    from .models import EvotorEvent, EvotorBulk
    from .client import EvotorAPICloud

    try:
        bulk = EvotorBulk.objects.filter(evotor_id=bulk_evotor_id).first()
        if not bulk:
        ...
        EvotorEvent.objects.create(
            sender=(
                ...
                else EvotorEvent.GROUP
            ),
            event_type=EvotorEvent.BULK,
        )
        ...
        if (
            bulk.status not in EvotorBulk.FINAL_STATUSES
...
def delete_evotor_product_task(product_evotor_id, is_parent, store_ids, user_id):
    try:
        logger.info(
            f"delete_evotor_product_task={product_evotor_id}, {is_parent}, {user_id}"
        )
        msg = EvatorCloud().delete_evotor_product_by_id(
            product_evotor_id, store_ids, is_parent
        )
        cache_key = f"user_message_{user_id}"
        cache.set(cache_key, msg, timeout=3600)
    except Exception as e:
...
def delete_evotor_additional_task(additional_evotor_id, store_ids, user_id):
    try:
        logger.info(
            f"delete_evotor_additional_task={additional_evotor_id}, {store_ids}"
        )
        msg = EvatorCloud().delete_evotor_additional_by_id(
            additional_evotor_id, store_ids
        )
        cache_key = f"user_message_{user_id}"
        cache.set(cache_key, msg, timeout=3600)
    except Exception as e:
