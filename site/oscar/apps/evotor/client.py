...
class EvotorAPICloud:
    ...
    def handle_bulks(self, response):
        ...
        bulk = EvotorBulk.objects.get_or_create(evotor_id=evotor_id)[0]
        bulk.status = response.get("status")
        bulk.object_type = response.get("type")
        bulk.save()

        if bulk.status in EvotorBulk.FINAL_STATUSES:
    ...
    def finish_bulk(self, bulk, response):
        ...
        if details:
            if bulk.object_type == EvotorBulk.PRODUCT:
                self.set_evotor_ids(details, Product, "article")
            elif bulk.object_type == EvotorBulk.PRODUCT_GROUP:
