...
class StoreSelectModalView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if basket_store and basket_store != cookie_store:
            ...
            response.set_cookie(
                "store", basket_store, max_age=settings.OSCAR_STORE_COOKIE_LIFETIME
            )
        elif not cookie_store and not basket_store:
            # Если данные о партнере отсутствуют в куки и корзине, устанавливаем по умолчанию
            response.set_cookie(
                "store", store_default, max_age=settings.OSCAR_STORE_COOKIE_LIFETIME
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            response.set_cookie(
                "store", form_store_id, max_age=settings.OSCAR_STORE_COOKIE_LIFETIME
            )
            return response
