...
class EvotorTablesMixin(MultiTableMixin, TemplateView):
    form_class = None
    ...
    def get_json(self, data_items, is_filtered):
        raise NotImplementedError("A EvotorTablesMixin must define a 'get_json' method")

    def process_items(self, data_items, is_filtered):
        raise NotImplementedError(
            "A EvotorTablesMixin must define a 'process_items' method"
        )
    ...
    def update_models(self, serializer, is_filtered):
        raise NotImplementedError(
            "A EvotorTablesMixin must define a 'update_models' method"
        )
    ...
    def send_models(self, serializer, is_filtered):
        raise NotImplementedError("A EvotorTablesMixin must define a 'send_models' method")
    ...
    def delete_models(self, delete_invalid):
        try:
            ...
            logger.error(
                "Ошибка при удалении моделей из бд на странице Эвотор - %s" % str(e)
            )
            messages.error(
                self.request,
